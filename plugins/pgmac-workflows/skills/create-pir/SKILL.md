...
**Execute all steps end-to-end — extract, analyse, write, create GitHub Issues, commit, and raise the PR. Do not just describe the steps or stop partway.**

Create a formal PIR document from the current Claude session, apply Infinite How's root cause analysis, evaluate and create runbooks for repeatable failure modes, create GitHub Issues for all actions, commit to a new branch, and raise a PR — all in a single automated flow.
...
- Severity: Critical / High / Medium / Low (severity guide is in the PIR template, read in Step 4)
...
For each distinct failure chain, drill from symptom to systemic root cause. Full methodology in `references/infinite-hows.md`.
...
### Decision: should a runbook be created or updated?

For each distinct root cause chain in the PIR, evaluate against these criteria:

**Create a new runbook when ALL of these are true:**
- The recovery procedure is repeatable — an on-call following the same steps would fix it again
- The failure mode has concrete, specific diagnostic commands (not just "check logs")
- The root cause is internal infrastructure (not a one-off external event, hardware failure, or upstream bug with no workaround)
- No existing runbook in `src/runbooks/` already covers this failure mode

**Add a failure mode to an existing runbook when:**
- An existing runbook covers the same service/symptom area
- The new failure mode produces the same or similar observable symptom
- The existing runbook would benefit from a new failure mode section

**Skip runbook creation when:**
- The failure is a true one-off (e.g., a disk physically died, a vendor had an outage)
- Recovery required a judgment call that can't be reduced to a procedure
- An existing runbook already covers this failure mode adequately
- The incident was caught and fixed so quickly that a runbook adds no practical value
...
`<service>-<failure-description>.md` — lowercase, hyphens, concise

Examples: `kubelet-silent-stall.md`, `dqlite-write-contention.md`, `calico-cni-unauthorized.md`
...
Read `references/github-issues-setup.md` first — it has the repo-selection table, issue description format, and title conventions.
...
Edit `/home/paul/pgmac/incidents/src/incidents/index.md`.

Insert a new row at the **top** of the table (newest-first):
```
...
- `/home/paul/pgmac/incidents/src/doc-templates/pir-template.md` — Full PIR document template with section-by-section guidance (also published at the incidents site under Templates → PIR Structure Template)
- `/home/paul/pgmac/incidents/src/doc-templates/runbook-template.md` — Runbook template covering simple and multi-mode patterns
